...
We fitted models on testing data and compared their MSE.Here are the bar plot of MSE in testing data of all methods and our shiny app's UI. The MSE of some non-parametric methods are near to variation of original methods and not as good as MSE of linear model. One reseason is that we didn't have enough memory to tune parameters by cross validation. So remaing results are based on default paramter in R package caret. 
