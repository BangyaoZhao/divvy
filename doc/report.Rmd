...
Even after fixing the transformation, the $MSE=40.58$ is still unsatisfactory, which means linear model may not be a perfect fit for this problem, so we decided to switch to other advanced non-parametric machine learning models. 

## Non-parametric Model and parallel computing in cluster

Due to the memory limitation, we first tried a single tree model, i.e., rpart, the Recursive Partitioning and Regression Trees model, to predict the duration of customers using Divvy bike. We used 20 cores to compute in parallel and used 5-fold cross validation to choose the best model. The procedure took one and a half hours. The MSE was 1143, which was worse than the linear model. Since the single tree model was not good enough, we then used ensemble tree model random forest, tree bag, and also knn, SVM with radial kernel. At first, we set memory as 50 GB in cluster but an out-of-memory issue came up. A trade-off was that the more cores we used, the faster the the programs would finish, but the more memory we would need. Therefore, we used 150 GB memory and still used 20 cores. We didn't have enough memory to tune parameters by cross validation. So remaing results are based on default paramter in R package caret.
...
We fitted models on testing data and compared their MSE.Here are the bar plot of MSE in testing data of all methods and our shiny app's UI.

![A screenshot of our shiny app]("../plots/rshiny.PNG")

```{r, echo = F}
library(ggplot2)
df=data.frame("Methods"=c("rpart"),"MSE"=c(1150.745))
ggplot(data=df) +
