...
The q-q plot is shown in Figure 3.
...
Due to the memory limitation, we first try a single tree model--rpart, which is Recursive Partitioning and Regression Trees, to predict the duration of customers using Divvy bike. We used 20 cores to compute in parallel and used 5 fold cross validation to choose the best model. The procedure costs one and half hours. The RMSE is 33.80845. The result is not as good as linear model.

Since single tree model is not good enough, we then used ensemble tree model random forest, tree bag and also knn, SVM with radial kernel, Gaussian Process model with radial kernel. At first, we set memory as 50GB in cluster but out of memory issure came up. Then we used 150BG memory and still used 20 cores. Due to time limitation, we don't have final fitted model of random forest, tree bag, SVM with radial kernel, Gaussian Process model with radial kernel. 

Fitted models saved in  mbox: https://umich.box.com/s/kxq930wc4pd32w348t8b1nwftd3o79dc.


### Prediction Results shown in interactive web apps

Please see file called shiny app.
...
![Figure 3. q-q Plot of the Linear Model](../plots/qqplot.png)
