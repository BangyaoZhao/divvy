...
Since single tree model is not good enough, we then used ensemble tree model random forest, tree bag and also knn, SVM with radial kernel, Gaussian Process model with radial kernel. At first, we set memory as 50GB in cluster but out of memory issure came up. Then we used 150BG memory and still used 20 cores. 

More results of rpart model saved in https://umich.box.com/s/kxq930wc4pd32w348t8b1nwftd3o79dc.
