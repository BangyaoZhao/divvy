...
data <-
  merge(data, divvydata2, by.x = "from_station_id", by.y = "ID")
# length(unique(data$from_station_id))
# 602
...
# divvydata <- read.csv("data/Divvy_Bicycle_Stations.csv")
loc <-
  divvydata[, c("ID", "Latitude", "Longitude")] # unique locations the stations
rownames(loc) <- loc[, 1]
loc <- loc[, 2:3]

cl10 <- sapply(seq(10, 100, 10), function(x) {
  # 10 to 100
...
divvydata <-
  merge(divvydata, cbind(ID = rownames(loc), clusterID = cl40$cluster),
        by = "ID")
...
#----
#unfactor
data$usertype = as.numeric(data$usertype)
